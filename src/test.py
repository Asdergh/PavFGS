...
# ViewMat[3, -1] = 500.0
K = torch.Tensor([[5.0, 0.0, 56.0], [0.0, 5.0, 56.0], [0.0, 0.0, 1]]).to(device)
...
model = VGGT(
    enable_point=True, enable_track=True, enable_depth=True, enable_camera=True
).to(device)
...
losses = []
frames_buffer = []
with tqdm(desc="GS Training ...", colour="green", ascii=":>", total=EPOCHS_N) as pbar:
...
path = "out_vis.gif"
frames_buffer[0].save(path, save_all=True, append_images=frames_buffer[1:], loop=0)
