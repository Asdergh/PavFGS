...
class TrainingConfig(NamedTuple):
    position_lr_init: Optional[float]=0.01
    feature_lr: Optional[float]=1.0
    opacity_lr: Optional[float]=0.1
    scaling_lr: Optional[float]=0.01
    rotation_lr: Optional[float]=0.01
    semantic_feature_lr: Optional[float]=0.01
    percent_dense: Optional[float]=1.32
...
class GaussianModel:
    ...
    def get_items(self) -> dict:
        return {
            ...
            "colors": (self._features_dc.squeeze(), self._features_rest)
        }
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, semantic_feature_size : int, speedup: bool):
        ...
        scales = torch.ones_like(fused_point_cloud)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        grads = self.xyz_gradient_accum / (self.denom  +  1e-12)
...
if __name__ == "__main__":
    ...
    gs = GaussianModel(3)
    basic_pcd = BasicPointCloud(
        points=np.zerosom.normal(0, 1, (100, 3)),
        colors=np.zerosom.zeros(100, 3),
        normals=np.zeros((100, 3))
    )
    gs.create_from_pcd(basic_pcd, 0.1, 100, 1.0)
    items = gs.get_items()
    for key in items:
        if not isinstance(items[key], tuple):
            print(key, items[key].size(), items[key].requires_grad)
        
        else:
            print(key, items[key][0].size(), items[key][1].size(), items[key][0].requires_grad, items[key][1].requires_grad)
            eval_colors = eval_sh(deg=3, sh=items[key][1].permute(0, 2, 1), dirs=items["means"])
            print(eval_colors.size())