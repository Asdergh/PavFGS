...
import rerun as rr
import torch
from open3d.geometry import KDTreeSearchParamHybrid, KDTreeSearchParamKNN, PointCloud
...
class BasicPointCloudScene:
    def __init__(
        ...
        base_K: Optional[np.ndarray] = np.array(
            [[56.0, 0.0, 56.0], [0.0, 56.0, 56.0], [0.0, 0.0, 1.0]]
        ),
        ...
        if self.base_Twc is not None and isinstance(self.base_Twc, np.ndarray):
    ...
    def create_from_colmap(
        ...
        search_param: Optional[
            str | KDTreeSearchParamKNN | KDTreeSearchParamHybrid
        ] = "knn",
            ...
            with open(points_f, "r") as file:
                ...
                for idx in range(partitions_n):
                    ...
                    for raw_idx in range(start_idx, end_idx):
                        ...
                        cam_fs = set(
                            [
                                cams[cams["IMAGE_ID"] == id]["NAME"].item()
                                for id in cam_ids
                            ]
                        )
        ...
        with tqdm(
            desc="Reading imgs ...", colour="green", ascii=":>", total=len(imgs_fs)
        ) as pbar:
            quats = []
            txyzs = []
            gt_imgs = []
            ...
            for idx, img_f in enumerate(imgs_fs):
                ...
                quat = torch.Tensor(
                    [float(val) for val in cam_raw.iloc[0, 1:5].tolist()]
                )
                txyz = torch.Tensor(
                    [float(val) for val in cam_raw.iloc[0, 5:8].tolist()]
                )
        ...
        if max_radii is not None:
            prune_mask = np.where(
                np.linalg.norm(points_, axis=-1) <= max_radii, True, False
            )
        ...
        self.viewmats = self._handle_poses(
            txyzs, quats, inv=inv_poses, axis_correction_dir=axis_correction_dir
        )
    ...
    def save_ply(self, path: str) -> None:
        ...
        if not os.path.exists(path):
            os.mkdir(path)
            pcd_f = os.path.join(path, "pcd_formar_scene.ply")
    ...
    def show(self) -> None:
        ...
        print(
            scene_items["colors"].shape,
            scene_items["colors"].min(),
            scene_items["colors"].mean(),
            scene_items["colors"].max(),
        )
        ...
        for idx, viewmat in enumerate(scene_items["viewmats"]):
            ...
            rr.log(
                ...
                rr.Transform3D(mat3x3=viewmat[:3, :3], translation=viewmat[:3, -1]),
            )
            rr.log(
    ...
    def get_items(self) -> dict:
        ...
        return {
            ...
            "bbox_center": (np.asarray(bbox.center) if bbox is not None else None),
            "bbox_extent": (np.asarray(bbox.extent) if bbox is not None else None),
            "bbox_rotation": (
                R.from_matrix(bbox.R).as_quat() if bbox is not None else None
            ),
