...
from open3d.geometry import PointCloud, KDTreeSearchParamKNN, KDTreeSearchParamHybrid
...
class BasicPointCloudScene:
    def __init__(
        ...
        base_K: Optional[np.ndarray] = np.array(
            [[5.0, 0.0, 56.0], [0.0, 5.0, 56.0], [0.0, 0.0, 1.0]]
        ),
    ) -> None:
    ...
    def create_from_tensor(
        ...
        if len(inputs.size()) == 5:
            ...
            for batch_idx in range(inputs.size()[0]):
                ...
                t, quats = vggt_item_["pose_enc"][batch_idx, :, :-2].split(
                    [3, 4], dim=-1
                )
            ...
            pcd = self._handle_frames_strack(
                inputs=inputs, pts=vggt_item_["world_points"], search_param=search_param
            )
            self.pcds = [pcd]
