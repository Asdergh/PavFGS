        ...
        for v in self.fv:
            if space_dim == "2d":
                if isinstance(v, tuple):
                    ...
                    block = nn.Sequential(
                        nn.Conv2d(in_f, v, (3, 3), 1, 1), hiden_act(), nn.BatchNorm2d(v)
                    )
                    in_f = v

            elif space_dim == "3d":
                if isinstance(v, tuple):
                    block = nn.Upsample(scale_factor=(2 ** v[0], 2 ** v[1], 2 ** v[2]))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for idx, block in enumerate(self.blocks):
            ...
            if self.res:
                if (
                    idx != 0
                    and not isinstance(self.fv[idx - 1], tuple)
                    and not isinstance(self.fv[idx], tuple)
                ):
                    x += features[-1]

            if self.rs:
                if (idx == len(self.blocks) - 1) or isinstance(self.fv[idx + 1], tuple):
                    sc_features += [
                        x,
                    ]
