...
from torch.optim import Adam, AdamW, Optimizer, SparseAdam
from torchvision.io import write_video
from torchvision.transforms import functional as Fv
from tqdm import tqdm

from src.scene.pts_scene import BasicPointCloudScene
from src.utils import DSSIMLoss, eval_sh

__optimizers__ = {"adam": Adam, "adamw": AdamW, "sparse_adam": SparseAdam}
...
class Config:
    ...
    save_ecposhs: Optional[List] = field(
...
class PavGSTrainer:
    def __init__(
        ...
        (self.splats, self.opt, self.gt_rgb, self.viewmats, scene_scale, self.Ks) = (
            self.parse_scene()
        )
        self.viewmats[..., :3, -1] = F.sigmoid(self.viewmats[..., :3, -1])
        print(
            self.viewmats[..., :3, -1].min(),
            self.viewmats[..., :3, -1].mean(),
            self.viewmats[..., :3, -1].max(),
        )
        print(
            self.viewmats[..., :3, :3].min(),
            self.viewmats[..., :3, :3].mean(),
            self.viewmats[..., :3, :3].max(),
        )
        print(
            self.splats["means"].min(),
            self.splats["means"].mean(),
            self.splats["means"].max(),
        )
    ...
    def parse_scene(
        self,
    ) -> Tuple[nn.ParameterDict, Union[Dict[str, Optimizer], Optimizer]]:
        ...
        Ks = (
            torch.Tensor(self.base_scene.K)[None]
            .repeat(gt_rgb.size()[0], 1, 1)
            .to(self.device)
        )
        ...
        splats_attrs = [
            ...
            (
                "scales",
                nn.Parameter(F.sigmoid(scales).to(self.device)),
                self.cfg.geom_lr,
            ),
            (
                "opacities",
                nn.Parameter(F.sigmoid(opactieis).to(self.device)),
                self.cfg.colors_lr,
            ),
        ...
        if self.cfg.optim_type.lower() in __optimizers__:
            ...
            raise ValueError(
                "config: optim_type can be only: [Adam, AdamW, Sparse_Adam]"
            )
        ...
        if self.cfg.split_gs_optimizers:
            ...
            opt = opt(
                [{"params": [p], "lr": p, "name": n} for (n, p, l) in splats_attrs],
                lr=0.1,
            )
    ...
    def train(self) -> None:
        ...
        if not self.cfg.mse_loss and not self.cfg.mae_loss and not self.cfg.dssim_loss:
            raise ValueError(f"""
            ...
            for idx in range(self.cfg.epochs_n):
                if self.cfg.apply_sh:
                    colors = eval_sh(
                        ...
                        sh=torch.cat(
                            [self.splats["sh0"], self.splats["shN"]], dim=1
                        ).permute(0, 2, 1),
                ...
                rendered_rgb, _, meta = rasterization(
                    ...
                    colors=colors,
                    width=self.base_scene.w,
                    height=self.base_scene.h,
                ...
                if self.cfg.save_samples:
                    ...
                    if self.cfg.dssim_loss:
                        _, ssim_heatmap = dssim_loss(
                            ref_gt, ref_render, get_ssim_map=True
                        )
                    ...
                    if idx != 0:
                        if self.cfg.densification_strat.lower() != "none":
                            if isinstance(self.strategy, DefaultStrategy):
                                ...
                                if (
                                    idx > self.cfg.densification_start
                                    and idx < self.cfg.densification_until
                                    and idx % self.cfg.densification_interval == 0
                                ):
                                    print(
                                        (32 * "=")
                                        + "... Denficiation results ..."
                                        + (32 * "=")
                                    )
                                    print(
                                        f"means: [{self.splats['means'].size()}, {self.splats['means'].min()}, {self.splats['means'].max()}]"
                                    )
                                    print(
                                        f"opacities: [{self.splats['opacities'].size()}, {self.splats['opacities'].min()}, {self.splats['opacities'].max()}]"
                                    )
                                    print(
                                        f"scales: [{self.splats['scales'].size()}, {self.splats['scales'].min()}, {self.splats['scales'].max()}]"
                                    )
                                    print(
                                        (32 * "=")
                                        + "... Denficiation results ..."
                ...
                if idx in self.cfg.save_ecposhs:
                    if self.cfg.save_samples:
                        ...
                        if self.cfg.dssim_loss:
                            map2save = (
                                torch.stack(self.ssim_maps2save, dim=0)
                                .detach()
                                .cpu()
                                .numpy()
                            )
                            ...
                            colorized_maps = (
                                255
                                * (
                                    colorized_maps.squeeze(dim=0).permute(0, -1, 1, 2)[
                                        :, :3, ...
                                    ]
                                )
                            )
