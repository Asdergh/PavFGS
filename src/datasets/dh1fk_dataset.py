...
class Dh1kDataset(BaseDataset):
    def __init__(
        ...
        self.avi_paths = [
            os.path.join(self.avi_root, path) for path in self.avi_paths[idx.tolist()]
        ]
        self.gts_paths = [
            os.path.join(self.gt_root, path) for path in self.gts_paths[idx.tolist()]
        ]
